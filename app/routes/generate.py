...
# Bounded in-memory history — last N question/answer pairs for this running instance.
# Not persisted across restarts; just enough to inspect recent activity.
...
def _cached_generate(prompt: str) -> str:
    """Cache identical prompts so repeat questions don't burn a fresh Vertex AI call.
    lru_cache keys purely on the prompt text, which is fine here since the
    system instruction and model are fixed for the app's lifetime."""
...
def generate_content():
    ...
    try:
        ...
        logger.info(
            "prompt_len=%d elapsed=%.3fs cache_hit=%s",
            len(prompt), elapsed, was_cache_hit
        )
...
def history():
    """Last few questions asked this session — handy for demoing or debugging."""
    return jsonify({"status": "success", "history": list(_history)}), 200


@generate_bp.route("/stats", methods=["GET"])
def stats():
    """Lightweight runtime telemetry: how many requests, how many served from cache."""
